  ...
  async getData(options) {
    ...
    for (const a in sheetData.system.attributes) {
      for (const s in sheetData.system.attributes[a].skills) {
        if (
          sheetData.system.attributes[a].skills[s].max === undefined ||
        ...
        if (
          sheetData.system.attributes[a].skills[s].value <=
          sheetData.system.attributes[a].skills[s].min
        ) {
          sheetData.system.attributes[a].skills[s].value =
            sheetData.system.attributes[a].skills[s].min;
        }
    ...
    if (sheetData.system.crew.length > 0) {
      ...
      sheetData.system.stress.max =
        sheetData.system.stress.max + crew_actor.system.scoundrel.add_stress;
      //add to max trauma
      sheetData.system.trauma.max =
        sheetData.system.trauma.max + crew_actor.system.scoundrel.add_trauma;
      //check for mastery
